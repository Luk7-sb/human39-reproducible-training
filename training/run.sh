...
if [[ -n "$motion_manifest" ]]; then
  ...
  eval_envs=$("$TRAIN_PYTHON" -c 'import json,sys; m=json.load(open(sys.argv[1])); assert m["complete"]; print(len(m["accepted"]))' "$motion_manifest")
fi
mkdir -p "runs/$run_name/source"
cp training/train.py training/human39_env.py training/run.sh "runs/$run_name/source/"
"$TRAIN_PYTHON" training/train.py --headless --device cuda:0 --num_envs "$num_envs" --iterations "$iterations" --run "$run_name" --max_hours "$max_hours" "${extra[@]}" "${manifest_extra[@]}"
latest_checkpoint=$(find "runs/$run_name" -maxdepth 1 -name 'model_*.pt' -printf '%T@ %p\n' | sort -n | tail -1 | cut -d' ' -f2-)
printf '%s evaluating\n' "$(date -Is)" > "runs/$run_name/state.txt"
"$TRAIN_PYTHON" training/train.py --headless --device cuda:0 --num_envs "$eval_envs" --run "${run_name}_eval" --checkpoint "$latest_checkpoint" --eval_steps 400 "${manifest_extra[@]}"
